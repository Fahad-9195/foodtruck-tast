...
import { getMyOwnerTrucks } from "@/features/trucks/api/trucks.api";
import { getIncomingOwnerOrders, updatePickupOrderStatus, type IncomingPickupOrder } from "@/features/orders/api/orders.api";
...
const statusTone = (status: string) => {
  switch (status) {
    case "pending":
      return { bg: colors.warningMuted, border: "rgba(255, 176, 32, 0.35)", fg: "#B45309" };
    case "preparing":
      return { bg: colors.primaryMuted, border: colors.borderStrong, fg: colors.primaryDark };
    case "ready":
      return { bg: colors.successMuted, border: "rgba(25, 195, 125, 0.35)", fg: colors.success };
    case "picked_up":
      return { bg: "rgba(123, 138, 160, 0.12)", border: colors.border, fg: colors.textSecondary };
    case "cancelled":
      return { bg: colors.dangerMuted, border: "rgba(255, 90, 107, 0.35)", fg: colors.danger };
    default:
...
export const OwnerIncomingOrdersScreen = () => {
  ...
  const toastTranslateY = useState(() => new Animated.Value(-8))[0];
  ...
  const updateStatusMutation = useMutation({
    ...
    onSuccess: async (_, variables) => {
      const nextStatusLabel = statusToArabic[variables.status] ?? variables.status;
      const movedFromFilter = filter !== "all" && filter !== variables.status;
      setFeedback({
        type: "success",
        message: movedFromFilter
          ? `تم تحديث الطلب إلى ${nextStatusLabel} وتم نقله تلقائيًا إلى تبويبه.`
          : `تم تحديث الطلب إلى ${nextStatusLabel}`
      });
      ...
      Animated.parallel([
        ...
        setTimeout(() => {
          Animated.parallel([
            ...
            Animated.timing(toastTranslateY, { toValue: -8, duration: 180, useNativeDriver: true })
    ...
    onError: (error) => {
      setFeedback({ type: "error", message: getReadableNetworkError(error) });
      setPendingConfirmation(null);
    }
  });
...
const styles = StyleSheet.create({
  ...
  feedbackError: {
    ...
    borderColor: "rgba(255, 90, 107, 0.4)"
  },
  feedbackText: {
    color: colors.text,
    fontSize: typography.caption,
